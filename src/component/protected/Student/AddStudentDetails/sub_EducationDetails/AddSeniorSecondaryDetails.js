...
function AddSeniorSecondaryDetails() {
    const boardArray = [
        {
            label: "CBSE",
            value: "CBSE"
    ...
    const submitDataa = ((e) => {
        ...
        try {
            firebase.database().ref("data").child('student').child(userId).child('educationDetails/seniorSecondary').set({
    ...
    return (
        <div className="backgroundPage">
            <div className="centerWholePage">
                <div className="centerPageHeading">Add Senoir Secondary (XII) <span className="colorChange"> Details </span> </div>
                        ...
                        <section className="rowSection">
                            <form className="detailsForm" onSubmit={submitDataa}>
                                <div className="detailsSection">
                                    ...
                                    <TextField onChange={(e) => setBoard(e.target.value)} className="textField" select required="true"
                                        style={{ minWidth: '200px' }} variant="standard" label="Board" value={board} size="normal">{boardArray.map((option) => (<MenuItem key={option.value} size="normal" value={option.value}> {option.label}</MenuItem>))}</TextField>
                                    ...
                                    <TextField onChange={(e) => setEndingYear(e.target.value)} value={endingYear} className="textField" select required="true"
...
export default AddSeniorSecondaryDetails
